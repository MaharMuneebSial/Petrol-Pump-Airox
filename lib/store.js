...
export const addMachine = async (machine) => {
  ...
  // Save initial nozzle readings
  if (machine.nozzleReadings && Object.keys(machine.nozzleReadings).length > 0) {
    const rows = Object.entries(machine.nozzleReadings).map(([num, reading]) => ({
      company_id,
      machine_id: data.id,
      nozzle_number: parseInt(num),
      initial_reading: parseFloat(reading) || 0,
    }));
