...
const getCompanyId = () => {
  ...
  try {
    return JSON.parse(sessionStorage.getItem('ps_company'))?.id || null;
  } catch {
    return null;
  }
};
...
export const getCompany = () => {
  if (typeof window === 'undefined') return null;
  const data = sessionStorage.getItem('ps_company');
  return data ? JSON.parse(data) : null;
};

export const saveCompany = (company) => {
  sessionStorage.setItem('ps_company', JSON.stringify(company));
};

export const isAuthenticated = () => {
  if (typeof window === 'undefined') return false;
  return !!sessionStorage.getItem('ps_session');
};
...
export const setSession = (pumpCode) => {
  sessionStorage.setItem('ps_session', JSON.stringify({ pumpCode, loginAt: new Date().toISOString() }));
};

export const clearSession = () => {
  sessionStorage.removeItem('ps_session');
  sessionStorage.removeItem('ps_company');
};
...
export const getMachines = async () => {
  ...
  const { data, error } = await supabase
    .from('machines')
    .select('*, machine_nozzles(nozzle_number, nozzle_name, product_id)')
    .eq('company_id', company_id)
    .order('name');
  if (error) { console.error('getMachines error:', error); return []; }
  return (data || []).map(row => ({
    ...mapMachine(row),
    nozzles: (row.machine_nozzles || []).sort((a, b) => a.nozzle_number - b.nozzle_number),
  }));
};
...
export const addMachine = async (machine) => {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      product_id: machine.productId || null,
      nozzle_count: parseInt(machine.nozzleCount || 2),
    })
    .select()
    .single();
  if (error) { console.error('addMachine error:', error.message, error.code, error.details); return null; }

  // Save nozzle config (staff, product, initial reading per nozzle)
  if (machine.nozzleConfig && Object.keys(machine.nozzleConfig).length > 0) {
    const rows = Object.entries(machine.nozzleConfig).map(([num, c]) => ({
      company_id,
      machine_id:      data.id,
      nozzle_number:   parseInt(num),
      nozzle_name:     c.nozzle_name || `Nozzle ${num}`,
      product_id:      c.product_id || null,
      initial_reading: parseFloat(c.initial_reading) || 0,
    }));
    const { error: nErr } = await supabase.from('machine_nozzles').insert(rows);
    if (nErr) console.error('machine_nozzles insert error:', nErr);
  }
...
export const updateSale = async (id, updates) => {
  // Fetch old sale to reverse its effects
  const { data: old } = await supabase.from('sales').select('product_id, account_id, quantity, total_amount, payment_mode').eq('id', id).single();
  if (!old) return null;

  const newProductId  = updates.productId  !== undefined ? updates.productId              : old.product_id;
  const newQty        = updates.quantity   !== undefined ? parseFloat(updates.quantity)   : parseFloat(old.quantity);
  const newRate       = updates.rate       !== undefined ? parseFloat(updates.rate)       : parseFloat(old.rate || 0);
  const newTotal      = updates.total      !== undefined ? parseFloat(updates.total)      : parseFloat(old.total_amount);
  const newMode       = updates.paymentMode !== undefined ? updates.paymentMode           : old.payment_mode;
  const newCustomerId = updates.customerId !== undefined ? (updates.customerId || null)   : old.account_id;

  // Reverse old product stock
  if (old.product_id) {
    const { data: oldProd } = await supabase.from('products').select('current_stock').eq('id', old.product_id).single();
    if (oldProd) await supabase.from('products').update({ current_stock: parseFloat(oldProd.current_stock || 0) + parseFloat(old.quantity || 0) }).eq('id', old.product_id);
  }

  // Apply new product stock
  if (newProductId) {
    const { data: newProd } = await supabase.from('products').select('current_stock').eq('id', newProductId).single();
    if (newProd) await supabase.from('products').update({ current_stock: parseFloat(newProd.current_stock || 0) - newQty }).eq('id', newProductId);
  }

  // Reverse old credit balance
  if (old.account_id && old.payment_mode === 'credit') {
    const { data: oldAcc } = await supabase.from('accounts').select('current_balance').eq('id', old.account_id).single();
    if (oldAcc) await supabase.from('accounts').update({ current_balance: parseFloat(oldAcc.current_balance || 0) - parseFloat(old.total_amount || 0) }).eq('id', old.account_id);
  }

  // Apply new credit balance
  if (newCustomerId && newMode === 'credit') {
    const { data: newAcc } = await supabase.from('accounts').select('current_balance').eq('id', newCustomerId).single();
    if (newAcc) await supabase.from('accounts').update({ current_balance: parseFloat(newAcc.current_balance || 0) + newTotal }).eq('id', newCustomerId);
  }

  const dbUpdates = {};
  if (updates.date        !== undefined) dbUpdates.sale_date    = updates.date;
  if (updates.productId   !== undefined) dbUpdates.product_id   = newProductId;
  if (updates.quantity    !== undefined) dbUpdates.quantity     = newQty;
  if (updates.rate        !== undefined) dbUpdates.rate         = newRate;
  if (updates.total       !== undefined) dbUpdates.total_amount = newTotal;
  if (updates.paymentMode !== undefined) dbUpdates.payment_mode = newMode;
  if (updates.customerId  !== undefined) dbUpdates.account_id   = newCustomerId;
  if (updates.note        !== undefined) dbUpdates.note         = updates.note || null;
...
export const deleteSale = async (id) => {
  const { data: sale } = await supabase.from('sales').select('product_id, account_id, quantity, total_amount, payment_mode').eq('id', id).single();
  let delProductName = 'Unknown';

  // Restore product stock
  if (sale?.product_id) {
    const { data: p } = await supabase.from('products').select('name, current_stock').eq('id', sale.product_id).single();
    if (p) {
      delProductName = p.name;
      await supabase.from('products').update({ current_stock: parseFloat(p.current_stock || 0) + parseFloat(sale.quantity || 0) }).eq('id', sale.product_id);
    }
  }

  // Reverse customer credit balance
  if (sale?.account_id && sale?.payment_mode === 'credit') {
    const { data: acc } = await supabase.from('accounts').select('current_balance').eq('id', sale.account_id).single();
    if (acc) await supabase.from('accounts').update({ current_balance: parseFloat(acc.current_balance || 0) - parseFloat(sale.total_amount || 0) }).eq('id', sale.account_id);
  }
...
export const updateExpense = async (id, updates) => {
  const descriptionField = updates.description
    ? `${updates.category}: ${updates.description}`
    : (updates.category || '');
  const { error } = await supabase
    ...
    .update({
      ...
      payment_mode:  updates.paymentMode || 'cash',
...
export const getExpenseCategories = async () => {
  ...
  const { data, error } = await supabase
    ...
    .eq('company_id', company_id)
    .order('name');
  if (error) { console.error('getExpenseCategories error:', error); return []; }
  return data || [];
};

export const addExpenseCategory = async (cat) => {
  const company_id = getCompanyId();
  if (!company_id) return null;
  const { data, error } = await supabase
    .from('expense_categories')
    .insert({ company_id, name: cat.name })
    .select()
    .single();
  if (error) { console.error('addExpenseCategory error:', error); return null; }
  return data;
};

export const deleteExpenseCategory = async (id) => {
  const { error } = await supabase.from('expense_categories').delete().eq('id', id);
  if (error) console.error('deleteExpenseCategory error:', error);
};
...
export const updateRateAdjustment = async (id, updates) => {
  const { error } = await supabase
    .from('rate_adjustments')
    .update({
      adjustment_date: updates.date,
      reason: updates.reason || null,
    })
    .eq('id', id);
  if (error) { console.error('updateRateAdjustment error:', error); return false; }
  return true;
};
